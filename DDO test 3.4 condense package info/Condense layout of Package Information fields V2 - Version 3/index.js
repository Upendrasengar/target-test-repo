...
\
    ...
    package.custom-package lithium-battery-options > base-option > div, package.custom-package reference-numbers > base-option > div {\
        border: 1px solid #eae4e4;\
        padding: 12px;\
    }\
    package.custom-package reference-numbers > base-option > div {\
        margin-bottom: 10px;\
    }\
    ...
    section > shipment-packages > package.custom-package > section > div.panel-body > div.ups-even.panel-body .custom-row div.ups-form_group > div > div.col-xs-1.text-left > spinner > img {\
     ...
     ';
        ...
        function updatePackageInfo() {
            ...
            if (!document.querySelector("#ewsSimpleRateCancelBannerText")) {
                ...
                if (!$("package div.row.custom-row").length) {
                    ...
                    // document.addEventListener("change", updatePackageOptions);
                    ...
                    //     if (event.target.id && ~event.target.id.indexOf("nbsPackagePackagingTypeDropdown")) {
                    //         setTimeout(function () {
                    //             updatePackageOptions();
                    //         }, 500);
                    //     }
                    ...
                    observerForPackageOptions.observe(document.querySelector("package-options"), {
                        childList: true,
                        subtree: true
                    });
                    ...
                    if (~window.location.href.indexOf("ups.com/ship/single-page")) {
                        $("package > section > div.panel-body.ng-star-inserted > div.ups-even.panel-body > div:nth-child(1) > div:nth-child(2)").removeClass("col-xs-6").addClass("col-xs-4").append('<div class="ups-package_image_container hidden-sm hidden-xs ng-star-inserted"><!----><img class="ups-package_image ng-star-inserted" src="/assets/resources/images/shipping_packages/custom-box-shipping-icons.svg" alt="My Packaging"><!----></div>');
        ...
        function updatePackageOptions() {
            // console.debug("update updatePackageOptions");
            setTimeout(function () {
                ...
                var lithiumOptions = $(".ups-accordion_list lithium-battery-options");
                if (lithiumOptions.length) {
                    $("lithium-battery-options").prependTo($("package.custom-package > section > div.panel-body > div.ups-section"));
                }
                $("lithium-battery-options label input").each(function (ind, ele) {
                    var input = ele;
            ...
            $("package.custom-package > section > div.panel-body > div.ups-section").addClass("other-options");
            $("reference-numbers").prependTo($("package.custom-package > section > div.panel-body > div.ups-section"));

            //Change package-options toggle to checkbox
            $("package-options").find(".ups-lever_label").removeClass("ups-lever_label");
            $("package-options label input").each(function (ind, ele) {
                var input = ele;
                if (!$(input).hasClass("ups-checkbox-custom")) {
                    $(input).addClass("ups-checkbox-custom").closest(".ups-toggle_listItem").addClass("ups-toggle_listItem ups-checkbox ups-input_wrapper");
                    $(input).closest("label").addClass("ups-checkbox-custom-label").find(".ups-lever_switch").remove();
                    $(input).prependTo($(input).closest("common-switch"));
                }
        ...
        function addToggle() {
            var accrodian = $("package > section > div.panel-body > div.ups-section").append('<section class="ups-accordion_list">\
          <div class="ups-accordion_wrapper">\
            <ul class="ups-accordion_content" style="border: 0;">\
              <li class="ups-accordion_item ups-active">\
                <button\
				  ...
				  id="packageOptionsDrawer"\
                  aria-expanded="true">\
                  <span class="hidden close-title">\
                    Show All Package Options\
                  </span>\
                  <span class="hidden open-title">\
                   Hide All Package Options\
                  </span>\
            ...
            $("package-options").appendTo(accrodian.find(".ups-accordion_expand"));
            // setTimeout(function () {
